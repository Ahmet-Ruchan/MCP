...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    if name == "generate_mcp_server":
        ...
        try:
            ...
            result = f"""✅ Successfully generated MCP server '{server_name}'!
...
"""
        ...
        templates = """📋 Available MCP Server Templates:

1️⃣ Tool Server (type: "tool")
...
2️⃣ Resource Server (type: "resource")
...
💡 Use 'generate_example_config' to see example configurations for each type.
"""
        ...
        if errors:
            ...
            result = f"⚠️  Validation passed with warnings:\n\n" + "\n".join([f"  - {w}" for w in warnings])
        else:
        ...
        if server_type in examples:
            ...
            result = f"""📝 Example configuration for '{server_type}' server:
...
"""
            ...
            result = f"❌ Unknown server type: {server_type}"
